...
function dump_public_section(  decl, def) {
    if (in_public == "if" || in_public == "define" ||
...
!in_tests && !in_public && /^\s*\/\*\s*public\s*\*\// {
    sub(/^\s*\/\*\s*public\s*\*\//, "");
    in_public = "start"
    public_section = ""
...
!in_public && /^\s*\/\*\s*testcase:.*\*\/\s*$/ {
    printf "#line %d \"%s\"\n", FNR + 1, FILENAME >tests_file
    test_descr = gensub(/^\s*\/\*\s*testcase:\s*(.*\S)\s*\*\/\s*$/, "\\1", "1");
    testdef = ""
    while (getline > 0) {
        testdef = testdef $0 "\n"
        if (/{/)
    ...
    if (test_args != "void")
    {
        ...
        for (i = 1; i <= n; i++)
        {
            ...
            arg_name = gensub(/^.*(\w+)$/, "\\1", "1", test_arg_list[i]);
...
in_public == "instantiate" && /^\s*#\s*include/ {
    dump_public_section();
}
